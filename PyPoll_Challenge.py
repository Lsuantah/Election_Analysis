...
with open(election_csv) as election_data:
    ...
    for row in reader:
        ...
        if county_name not in county_names:
            ...
            counties_Votes_dict[county_name] = 0

        # 5: Add a vote to that county's vote count.
        counties_Votes_dict[county_name] += 1
...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in counties_Votes_dict:

        # 6b: Retrieve the county vote count.
    
        county_vote = counties_Votes_dict[county_name]
        ...
        if(county_vote > winning_count) and (vote_percentage > winning_percentage):
    ...
    f"--Largest County Turnourt-------------\n"
    f"County: {winning_county}\n"
    f"County Vote Count: {winning_count:,}\n"
    f"County Percentage: {winning_percentage:.1f}%\n"
    f"-------------------------\n")
    ...
    for candidate_name in candidate_votes:
        ...
        if (votes >= county_winning_count) and (vote_percentage >= county_winning_percentage):
            ...
            county_winning_candidate = candidate_name
            county_winning_percentage = vote_percentage

    # Print the winning candidate (to terminal)
    winning_candidate_summary = (
        f"-------------------------\n"
        f"Winner: {county_winning_candidate}\n"
